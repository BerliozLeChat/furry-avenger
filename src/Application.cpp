...
int main(){
	...
	cout<<"[{ Bon soit, et que donnerait une fusion des deux ?: 11 elements attendus }]"<<endl;
		...
		Camille->afficher();
	cout<<"[{ D accord... mais si on enleve tous ce qui fait Antoine de Camille... ?: 1 element attendu }]"<<endl;
		Camille->enleve(*Antoine);
		Camille->afficher();
	cout<<"[{ interessant... et l inverse, ca doit donc donner... ?: 0 elements attendu }]"<<endl;
	...
	cout<<"[{ Bien, maintenant est-il possible d'avoir autre chose qu un ensemble vide avec Pierre si on lui ote un element au hasard ?: 0 element attendu }]"<<endl;
		Pierre->oteTous(3);
		Pierre->afficher();
	cout<<"[{ Ajoutons des éléments à Pierre ?: resultat 3 attendu |resultat 1 attendu |resultat 0 attendu }]"<<endl;
		Pierre->ajouter(1);
		Pierre->ajouter(1);
		Pierre->ajouter(1);
		Pierre->ajouter(5);
		cout<<Pierre->nbOcc(1)<<"|"<<Pierre->nbOcc(5)<<"|"<<Pierre->nbOcc(8);
